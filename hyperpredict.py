...
class HyperPredictLightningModule(pl.LightningModule):
    ...
    def forward(self, batch):
        if self.registration_model == "clapirn":
            ...
            with torch.no_grad():
                ...
                moving_img, fixed_img, target_dice, dice_mean,jdet_mean, jdet_std, target_jac, bending_energy, spacing= batch
                ...
                print("hyperparameters going in, bending energy before: ", bending_energy, "bending_energy after: ", torch.where(bending_energy > 0, bending_energy.log(), (bending_energy + 1e-5).log()), "spacing before: ", spacing, "spacing after: ", spacing.log())
                mapped_spacing = self.mapping(spacing.log())
                mapped_bending_energy = self.mapping(torch.where(bending_energy > 0, bending_energy.log(), (bending_energy + 1e-5).log()))
                
                 #global statistics
                if self.encoding_type == "mean_encoding":
                    global_mean = self.global_statistics(enc_rep, self.encoding_type)
                    enc_rep_cat = torch.cat([global_mean, mapped_spacing, mapped_bending_energy], dim= 1)

                elif self.encoding_type == "mean_min_max_encoding":
                    global_mean, global_max, global_min = self.global_statistics(enc_rep, self.encoding_type)
                    enc_rep_cat = enc_rep_cat = torch.cat([global_mean, global_max, global_min, mapped_spacing, mapped_bending_energy], dim= 1)
    ...
    def test_niftyreg(self, pair_idx, batch, bending_energy, spacing, nfv_percent):
        ...
        with torch.no_grad():
            ...
            for i in bending_energy:
                be = torch.tensor(i, device=self.device).float().unsqueeze(0).unsqueeze(0)
                sx = torch.tensor(spacing, device=self.device).float().unsqueeze(0).unsqueeze(0)

                mapped_log_be = self.mapping(torch.where(be > 0, be.log(), (be + 1e-5).log()))
                mapped_log_sx = self.mapping(sx.log())

                if self.encoding_type == "mean_encoding":
                    enc_rep_cat = torch.cat([global_mean, mapped_log_sx, mapped_log_be], dim= 1)
                elif self.encoding_type == "mean_min_max_encoding":
                    enc_rep_cat = torch.cat([global_mean, global_max, global_min, mapped_log_sx, mapped_log_be], dim= 1)
    ...
    def global_statistics(self, encoded_representation, encoding_type):
        ...
        global_mean = torch.mean(flattened_enc_rep, dim=2) # 1, 16 or 1, 56
        global_max = torch.max(flattened_enc_rep, dim=2)[0] # 1, 16
        global_min = torch.min(flattened_enc_rep, dim=2)[0] # 1, 16
